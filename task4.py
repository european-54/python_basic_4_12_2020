...
оргтехники (принтер, сканер, ксерокс). В базовом классе определить параметры, общие для приведенных
...
class Printer(Warehouse):
    def to_print(self):
        return f'to print the waiting {self.numb} times'


class Scanner(Warehouse):
    def to_scan(self):
        return f'to scan the waiting {self.numb} times'


class Copier(Warehouse):
    def to_copier(self):
        return f'to copier the waiting {self.numb} times'


unit_1 = Printer('Canon', 12000, 8, 16)
unit_2 = Scanner('Brother', 2300, 6, 34)
unit_3 = Copier('Xerox', 3500, 3, 23)
